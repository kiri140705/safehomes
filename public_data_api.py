...
def _require_env(name: str) -> str:
    ...
    if not value:
        raise RuntimeError(
            f"[SafeHomes] 환경변수 {name}가 설정되어 있지 않습니다. "
            f".env.example을 복사해 .env를 만들고 실제 키 값을 채워주세요."
        )
    return value
...
class PublicDataFetcher:
    def __init__(self):
        ...
        # 실제 API 발급 키는 하드코딩하지 않고 .env(환경변수)에서 읽어온다.
        self.portal_api_key = _require_env("PORTAL_API_KEY")
        self.vworld_api_key = _require_env("VWORLD_API_KEY")
        self.seoul_api_key = _require_env("SEOUL_API_KEY")
        self.law_api_key = _require_env("LAW_API_KEY")  # 국가법령정보센터 API 인증키
        self.reb_stats_key = _require_env("REB_STATS_KEY")  # 한국부동산원 통계 API

        # ScraperAPI 키 로테이션 (네이버 부동산 크롤러용, 단일 키 쿼터 소진/차단 방지)
        scraperapi_keys = [
            os.getenv(f"SCRAPERAPI_KEY_{i}") for i in range(1, 9)
        ]
        self.scraperapi_keys = [k for k in scraperapi_keys if k]
        if not self.scraperapi_keys:
            raise RuntimeError(
                "[SafeHomes] SCRAPERAPI_KEY_1 등 ScraperAPI 키가 .env에 하나도 설정되어 있지 않습니다."
            )
        self._scraperapi_key_cycle = itertools.cycle(self.scraperapi_keys)

    def _next_scraperapi_key(self) -> str:
        """ScraperAPI 키를 순환 배정하여 여러 키에 부하를 고르게 분산시킨다."""
        return next(self._scraperapi_key_cycle)
    ...
    _SIDO_ALIASES = {
        ...
        "강원": "강원", "충북": "충북", "충청북도": "충북", "충남": "충남", "충청남도": "충남",
        "전북": "전북", "전라북도": "전북", "전남": "전남", "전라남도": "전남",
        "경북": "경북", "경상북도": "경북", "경남": "경남", "경상남도": "경남", "제주": "제주",
    }

    def check_flood_risk(self, address: str):
        """행정안전부_통계연보_지역별 자연재해위험개선지구 지정 현황 실시간 연동.
        시/도 단위로 아직 정비되지 않은 침수위험지구 수를 조회해 위험도를 판단한다."""
        url = "http://apis.data.go.kr/1741000/DesiNatuDiRiskImprovDistrReg/getDesiNatuDiRiskImprovDistrReg"
        sido = None
        for alias, canon in self._SIDO_ALIASES.items():
            if alias in (address or ""):
                sido = canon
                break

        if not sido:
            return {"is_flood_zone": False, "warning": ""}

        try:
            params = {"serviceKey": self.portal_api_key, "pageNo": 1, "numOfRows": 200, "type": "json"}
            res = requests.get(url, params=params, timeout=5)
            if res.status_code != 200:
                return {"is_flood_zone": False, "warning": ""}
            rows = res.json().get("DesiNatuDiRiskImprovDistrReg", [None, {}])[1].get("row", [])
            if not rows:
                return {"is_flood_zone": False, "warning": ""}

            latest_year = max(r["wrttimeid"] for r in rows)
            matched = next((r for r in rows if r["wrttimeid"] == latest_year and r.get("regi") == sido), None)
            if not matched:
                return {"is_flood_zone": False, "warning": ""}

            fld_target = int(matched.get("fld_dng_orgnz_target") or 0)
            total_designated = int(matched.get("smry") or 0)

            if fld_target > 0:
                warning = (
                    f"⚠️ [행정안전부 자연재해위험개선지구 실통계 {latest_year}년 기준] {sido} 지역은 "
                    f"침수위험 정비대상 지구가 {fld_target}곳 남아있습니다 (누적 지정 {total_designated}곳). "
                    f"계약 전 반드시 정확한 소재지가 지정 구역에 포함되는지 지자체(시·군·구청)에 확인하십시오."
                )
                return {"is_flood_zone": True, "warning": warning}
            return {
                "is_flood_zone": False,
                "warning": f"ℹ️ [행정안전부 자연재해위험개선지구 실통계 {latest_year}년 기준] {sido} 지역은 현재 미정비 침수위험지구가 없는 것으로 집계됩니다 (지역 단위 통계이며 개별 필지 안전을 보장하지는 않습니다)."
            }
        except Exception as e:
            print(f"[!] 자연재해위험개선지구 API 호출 실패: {e}")
            return {"is_flood_zone": False, "warning": ""}
    ...
    def _fetch_reb_index_change(self, statbl_id: str, region: str):
        """한국부동산원 R-ONE 월별 지수 통계(SttsApiTblData.do)에서 최신월/직전월을 자동 탐색해
        실제 지수값과 전월 대비 변동률(%)을 계산해서 반환한다. (지가지수/매매·전세·월세가격지수 등 공용)
        반환: {"month_id": "202606", "region_label": str, "index": float, "change_pct": float|None} 또는 None"""
        import datetime

        url = "https://www.reb.or.kr/r-one/openapi/SttsApiTblData.do"

        def _fetch_month(yyyymm: str):
            params = {
                "KEY": self.reb_stats_key,
                "STATBL_ID": statbl_id,
                "DTACYCLE_CD": "MM",
            ...
            try:
                ...
                if res.status_code != 200:
                    return None
                return res.json().get("SttsApiTblData", [None, {}])[1].get("row", [])
            except Exception as e:
                print(f"[!] 한국부동산원 통계 API 호출 실패({statbl_id}, {yyyymm}): {e}")
                return None

        def _find_region_row(rows, region_keyword):
            if not rows:
                return None
            if not region_keyword or region_keyword in ["전국", "전체"]:
                for r in rows:
                    if r.get("CLS_NM") == "전국":
                        return r
                return None
            candidates = [r for r in rows if region_keyword in (r.get("CLS_FULLNM") or "")]
            return candidates[0] if candidates else None

        # 최신 발표월을 자동 탐색 (통상 1개월 정도 발표 지연이 있다)
        now = datetime.datetime.now()
        this_month_rows = None
        this_month_id = None
        for back in range(0, 4):
            ...
            if rows:
                this_month_rows = rows
                this_month_id = yyyymm
                break

        if not this_month_rows:
            return None

        row_now = _find_region_row(this_month_rows, region)
        if not row_now or row_now.get("DTA_VAL") is None:
            return None

        # 직전월 데이터로 실제 변동률 계산
        y, m = int(this_month_id[:4]), int(this_month_id[4:])
        m -= 1
        if m <= 0:
            m += 12
            y -= 1
        prev_month_id = f"{y}{m:02d}"
        prev_rows = _fetch_month(prev_month_id)
        row_prev = _find_region_row(prev_rows, region) if prev_rows else None

        idx_now = row_now.get("DTA_VAL")
        region_label = row_now.get("CLS_FULLNM") or row_now.get("CLS_NM") or region
        change_pct = None
        if row_prev and row_prev.get("DTA_VAL"):
            idx_prev = row_prev["DTA_VAL"]
            change_pct = (idx_now - idx_prev) / idx_prev * 100

        return {
            "month_id": this_month_id,
            "region_label": region_label,
            "index": idx_now,
            "change_pct": change_pct,
        }

    def fetch_land_price_trend(self, region: str) -> str:
        """한국부동산원 R-ONE 실시간 지가지수 연동 (전국지가변동률조사, STATBL_ID=A_2024_00901 월별 지역별 지가지수).
        최신월과 직전월 지수를 비교해 실제 지가변동률(%)을 계산해서 돌려준다. 데이터가 없으면 None."""
        result = self._fetch_reb_index_change("A_2024_00901", region)
        if not result:
            return None

        month_id = result["month_id"]
        year, month = month_id[:4], int(month_id[4:])
        if result["change_pct"] is not None:
            trend = "상승" if result["change_pct"] > 0 else ("하락" if result["change_pct"] < 0 else "보합")
            return (
                f"📊 [한국부동산원 실시간 지가지수] {result['region_label']} 지가지수는 {year}년 {month}월 기준 "
                f"{result['index']:.2f}(2026.01=100.0)로, 전월 대비 {result['change_pct']:+.2f}% {trend}했습니다."
            )
        return (
            f"📊 [한국부동산원 실시간 지가지수] {result['region_label']} 지가지수는 {year}년 {month}월 기준 "
            f"{result['index']:.2f}(2026.01=100.0)입니다. (직전월 비교 데이터는 확인되지 않았습니다)"
        )

    def get_macro_real_estate_stats(self, region_keyword: str) -> str:
        """[엔진 A: 거시경제 퀀트 분석기] - 한국부동산원 아파트 매매/전세/월세가격지수 실시간 연동 4대 국면 판독"""
        print(f"[*] 한국부동산원 거시경제 API '{region_keyword}' 매매/전세가지수 스캔 중...")

        # 한국부동산원 R-ONE 실데이터로 매매/전세/월세가격지수 전월 대비 변동률을 실제 계산한다.
        # (구 코드는 이 API 호출 결과를 버리고 지역명 키워드만으로 지어낸 고정 수치를 출력했었다)
        mae = self._fetch_reb_index_change("A_2024_00045", region_keyword)   # 아파트 매매가격지수
        jeonse = self._fetch_reb_index_change("A_2024_00050", region_keyword)  # 아파트 전세가격지수
        wolse = self._fetch_reb_index_change("A_2024_00055", region_keyword)   # 아파트 월세가격지수

        def _fmt(stat):
            if stat and stat.get("change_pct") is not None:
                return f"{stat['change_pct']:+.2f}%"
            return "데이터 확인 중"

        mae_pct, jeonse_pct, wolse_pct = _fmt(mae), _fmt(jeonse), _fmt(wolse)
        data_source_label = (mae or jeonse or wolse or {}).get("region_label", region_keyword)
        month_id = (mae or jeonse or wolse or {}).get("month_id")
        month_label = f"{month_id[:4]}년 {int(month_id[4:])}월" if month_id else "최신"

        # 상세 데이터 마크다운 템플릿
        disclaimer = "\n\n---\n*※ 본 브리핑은 한국부동산원 아파트 매매/전세/월세가격지수 실데이터를 기반으로 작성된 AI 애널리스트의 동향 분석 리포트입니다. 최종적인 부동산 매매 및 계약에 대한 결정권과 책임은 전적으로 본인에게 있으며, 본 시스템은 법적 책임을 지지 않습니다.*"
        index_line = (
            f"**2. 📊 [3대 지수 상관관계 분석 - {data_source_label} {month_label} 기준 실데이터]**\n"
            f"한국부동산원 R-ONE 통계에 따르면, 매매가지수는 전월대비 {mae_pct}, 전세가지수는 전월대비 {jeonse_pct}, "
            f"월세가격지수는 전월대비 {wolse_pct}를 기록했습니다.\n\n"
        )
        ...
        if "강남" in region_keyword or "서초" in region_keyword or "송파" in region_keyword:
            return (
                ...
                f"**4. 🔥 [세이프홈즈 시장 동향 결론]**\n"
                f"위 실데이터 흐름을 참고하여 향후 공급 물량, 정책 자금 쏠림 등 변수를 함께 점검하시고, 철저한 자금 조달 계획을 바탕으로 시장의 흐름을 예의주시하시기 바랍니다."
                + disclaimer
            )
            ...
            return (
                f"📉 **[세이프홈즈 수석 애널리스트 거시경제 브리핑: {region_keyword}]**\n\n"
                f"**1. 🌐 [거시경제 및 지역 시황 총평]**\n"
                f"현재 {region_keyword} 지역의 부동산 시장은 고금리 장기화에 따른 매수 심리 위축과 누적된 입주 물량의 여파로 인해 **[역전세 위험 점검이 필요한]** 국면이 관측되고 있습니다.\n\n"
                + index_line +
                f"**3. ⚖️ [정책 및 리스크 점검]**\n"
                f"매매가와 전세가의 방향성이 엇갈릴 경우 '전세가율 급등' 시점이 임박할 수 있어, 무자본 갭투자 매물들의 연쇄 부도(깡통전세) 리스크를 점검해야 합니다. 특히 HUG 전세보증금 반환보증 한도 축소로 인해 시장 내 융통 자금이 경색되고 있습니다.\n\n"
                f"**4. 🔥 [세이프홈즈 시장 동향 결론]**\n"
                f"위 실데이터 흐름을 근거로, 전세 시장으로 진입할 경우 보증금 미반환 리스크(깡통전세)를 반드시 점검하시고, 등기부등본 확인 및 HUG 보증보험 100% 가입 여부를 계약의 최우선 전제 조건으로 삼아야 합니다."
                + disclaimer
            )
            ...
            return (
                f"🧊 **[세이프홈즈 수석 애널리스트 거시경제 브리핑: {region_keyword}]**\n\n"
                f"**1. 🌐 [거시경제 및 지역 시황 총평]**\n"
                f"현재 {region_keyword} 지역의 부동산 시장은 영끌족의 투매 물량 증가와 매수세 위축 여부를 함께 점검해야 하는 **[하방 압력 점검 국면]**입니다.\n\n"
                + index_line +
                f"**3. ⚖️ [정책 및 리스크 점검]**\n"
                f"과거 저금리 시절 고점 부근에서 유입된 '영끌' 자본이 고금리를 견디지 못해 임의경매로 넘어가는 사례가 늘어날 수 있는 구간입니다. 하반기 대출 규제 강화 여부를 함께 모니터링해야 합니다.\n\n"
                f"**4. 🔥 [세이프홈즈 시장 동향 결론]**\n"
                f"위 실데이터 흐름을 근거로, 임대차 시장으로 진입하시더라도 추후 주택이 경매에 넘어갔을 때 보증금을 안전하게 지킬 수 있도록 전세 비중을 최소화하고 보증부 월세(반전세) 형태로 현금 유동성을 방어하는 전략을 고려하십시오."
                + disclaimer
            )
        else:
            return (
                ...
                f"**1. 🌐 [거시경제 및 지역 시황 총평]**\n"
                f"현재 {region_keyword} 지역의 부동산 시장은 아래 실데이터를 기준으로 매매·전세 방향성을 함께 점검해야 하는 국면입니다.\n\n"
                + index_line +
                f"**3. ⚖️ [정책 및 리스크 점검]**\n"
                f"매매가와 전세가의 방향성이 엇갈리는 '디커플링' 조짐이 보일 경우, 실수요 펀더멘털 없이 가격만 오르는 구간일 수 있으므로 거시경제의 작은 외부 충격(추가 금리 인상, 대출 만기 연장 거부 등)에 유의해야 합니다.\n\n"
                f"**4. 🔥 [세이프홈즈 시장 동향 결론]**\n"
                f"위 실데이터를 참고하여 무리한 레버리지를 지양하고, 매매/전세 지수의 방향성이 일치하는지 반드시 확인한 뒤 신중하게 접근하시기 바랍니다."
                + disclaimer
            )
    ...
    def fetch_applyhome_competition_rate(self, house_manage_no: str) -> str:
        """한국부동산원 청약홈 청약접수 경쟁률 실시간 연동 (신청 마감 후에만 공개된다).
        아직 접수 중이거나 데이터가 없으면 None."""
        ...
        try:
            ...
            if not rows:
                return None
            # 해당지역(01) 1순위 기준으로 가장 경쟁이 치열한 평형 순으로 요약
            rank1_local = [r for r in rows if r.get("RESIDE_SECD") == "01" and r.get("SUBSCRPT_RANK_CODE") == 1]
            target_rows = rank1_local or rows
            lines = []
            for r in target_rows[:5]:
                house_ty = (r.get("HOUSE_TY") or "").strip()
                req_cnt = r.get("REQ_CNT", "0")
                sply = r.get("SUPLY_HSHLDCO", "?")
                rate = r.get("CMPET_RATE", "-")
                lines.append(f"  · {house_ty}㎡ (공급 {sply}세대) 신청 {req_cnt}건 → 경쟁률 {rate}")
            if not lines:
                return None
            return "📈 [한국부동산원 청약홈 실시간 경쟁률]\n" + "\n".join(lines)
        except Exception as e:
            print(f"[!] 청약홈 경쟁률 API 호출 실패: {e}")
            return None

    def fetch_applyhome_area_stats(self, region: str) -> str:
        """한국부동산원 청약홈 지역별 연령대 청약 신청 통계 실시간 연동 (최근 발표월 자동 탐색)."""
        import datetime
        ...
        for back in range(0, 4):
            ...
            try:
                ...
                if not rows:
                    continue
                matched = None
                if not region or region in ["전국", "전체"]:
                    matched = rows[0]
                else:
                    matched = next((r for r in rows if region in (r.get("SUBSCRPT_AREA_CODE_NM") or "")), None)
                if not matched:
                    continue
                area_nm = matched.get("SUBSCRPT_AREA_CODE_NM", region)
                a30 = matched.get("AGE_30", 0)
                a40 = matched.get("AGE_40", 0)
                a50 = matched.get("AGE_50", 0)
                a60 = matched.get("AGE_60", 0)
                total = (a30 or 0) + (a40 or 0) + (a50 or 0) + (a60 or 0)
                if total == 0:
                    continue
                return (
                    f"👥 [한국부동산원 청약홈 실시간 청약 신청 통계] {area_nm} {y}년 {m}월 기준 "
                    f"청약 신청자 총 {total:,}명 (30대 {a30:,} / 40대 {a40:,} / 50대 {a50:,} / 60대 이상 {a60:,})"
                )
            except Exception as e:
                print(f"[!] 청약홈 통계 API 호출 실패({stat_de}): {e}")
                continue
        return None

    def fetch_sh_vacancy_and_plans(self, region: str, query: str = ""):
        """SH공사(서울) 주택 유형별 정밀 스캔 (국민임대 공급계획, 장기전세, 전세임대, 청년주택 분리)"""
        sh_key = self.seoul_api_key
        notices = []
        ...
        is_national_rent = any(k in query for k in ["국민임대"])
        ...
        if "전세" in query:
            ...
            fetch_all = not (is_youth or is_longterm or is_lease or is_national_rent)
        ...
        try:
            ...
            if is_longterm or fetch_all:
                ...
                if res_p.status_code == 200:
                    ...
                    for item in data_p:
                        loc = item.get("LCTN_RGN", "")
                        name = item.get("CMPX_NM", "").strip()
            ...
            # 3. 국민임대주택 공급계획 스캔
            if is_national_rent or fetch_all:
                url_national = f"http://openapi.seoul.go.kr:8088/{sh_key}/json/ctyDvlpRentHouse/1/50/"
                res_n = requests.get(url_national, timeout=5)
                if res_n.status_code == 200:
                    data_n = res_n.json().get("ctyDvlpRentHouse", {}).get("row", [])
                    for item in data_n:
                        loc = item.get("LCTN_RGN", "")
                        name = item.get("CMPX_NM", "").strip()
                        if not region or region == "전국" or region in loc or region in name:
                            search_q = urllib.parse.quote(f"SH 국민임대 {name} 공급 공고")
                            year = item.get("SPLY_PRNMNT_YR", "")
                            month = item.get("SPLY_PRNMNT_MM", "")
                            notices.append({
                                "id": f"SH_NATL_{name}",
                                "title": f"📅 [공급 예고] {loc} {name} 국민임대주택 공급 예정 ({year}년 {month})",
                                "url": f"https://search.naver.com/search.naver?query={search_q}",
                                "date": "공급계획"
                            })
        except Exception as e:
    ...
    def get_legal_precedent(self, property_type: str) -> str:
        ...
        # 검색 키워드(첫 번째 값)는 법제처 lawSearch.do 실검색에서 0건이 나오지 않도록
        # 사건명에 실제로 매칭되는 짧은 법률 용어로 검증해서 골랐다 (원래 문구는 다변화용 라벨).
        PRECEDENT_DB = {
            "상가": [
                ("권리금", "대법원 2019.5.16. 선고 2017다225312 (권리금 회수 방해): '임대인의 방해로 권리금 회수가 불가능해진 경우 손해배상 책임이 있다.' 👉 [방어 특약]: 임대인의 귀책사유나 변심으로 신규 임차인 주선 거절 시, 임차인이 산정한 권리금 전액을 즉시 현금 배상한다."),
                ("임대차해지", "대법원 2014.2.27. 선고 2013다95964 (3기 연체 해지): '차임 연체액이 3기에 달하면 해지 가능.' 👉 [방어 특약]: 건물 하자로 인한 영업 중단 기간 동안의 차임은 면제하며, 이를 이유로 한 임대인의 해지권은 제한된다."),
                ("원상회복", "대법원 1990.10.30. 선고 90다카12035 (원상회복 범위): '이전 임차인이 설치한 시설까지 철거할 의무 없음.' 👉 [방어 특약]: 원상회복은 현 임차인이 설치한 부속물에 한정하며, 종전 임차인의 시설물 철거 비용은 임대인이 부담한다."),
                ("업종제한", "대법원 2012.11.29. 선고 2011다79258 (독점 상가 보호): '업종 제한 약정 위반 시 동종 영업 금지 청구 가능.' 👉 [방어 특약]: 건물 내 동종 업종 입점 묵인 시 본 계약을 해제하고 권리금/인테리어 전액의 배액배상을 청구한다."),
                ("이행강제금", "대법원 2014.11.13. 선고 2014다236830 (불법건축물): '불법건축물 고지 의무 위반.' 👉 [방어 특약]: 건물 내 위반건축물 이력으로 인한 영업허가 지연 및 이행강제금 부과 시 그 피해 전액은 임대인이 배상한다."),
                ("정화조", "하급심 판례 (정화조 용량 부족): '용도 변경 시 정화조 원인자부담금.' 👉 [방어 특약]: 본 계약 체결 후 업종 진입에 필요한 정화조 원인자부담금 및 하수도법 관련 추가 비용은 임대인이 100% 부담한다.")
            ],
            "주택": [
                ("대항력", "대법원 2010.5.13. 선고 2009다97079 (대항력 발생 시기): '대항력은 전입신고 익일 0시 발생, 근저당은 당일 발생.' 👉 [방어 특약]: 임대인은 잔금 지급일 익일 자정까지 권리변동을 일으키지 않으며, 위반 시 계약 원천 무효 및 배액 배상한다."),
                ("신탁", "대법원 2017.12.21. 선고 2017다220744 (신탁 불법 임대차): '수탁자 동의 없는 임대차는 무효.' 👉 [방어 특약]: 본 계약은 수탁자(신탁회사)와 우선수익자의 사전 서면 동의를 조건으로 하며, 미동의 시 즉각 무효로 한다."),
                ("동시이행", "대법원 2002.2.26. 선고 2001다77697 (동시이행 항변권): '보증금 미반환 시 이사 안 가면 월세 면제.' 👉 [방어 특약]: 만기일 보증금 반환 지연 시, 임대인은 지연일수 1일당 보증금의 연 15%에 해당하는 지연 이자를 가산하여 지급한다."),
                ("전세보증금반환보증", "금융감독원 분쟁조정례 (HUG 보증거절): '보증보험 가입 불가 깡통전세.' 👉 [방어 특약]: 전세보증금반환보증(HUG 등) 가입이 임대인이나 목적물의 하자로 거절될 경우 계약은 원천 무효로 하며 계약금을 즉시 반환한다."),
                ("수선의무", "대법원 2012.3.29. 선고 2011다100226 (임대인의 수선의무): '대규모 수선은 임대인 책임.' 👉 [방어 특약]: 보일러, 누수, 곰팡이 등 건축물 노후화로 인한 중대한 하자는 입주 후라도 임대인이 전액 수리한다.")
            ],
            "토지": [
                ("토지매매", "대법원 2012.11.29. 선고 2012다69654 (맹지 기망): '건축 불가 맹지 기망 시 계약 취소.' 👉 [방어 특약]: 매수인의 목적(건축 인허가)이 지자체 규제나 맹지로 인해 불가할 시 조건 없이 무효로 하며 계약금 전액을 즉각 반환한다."),
                ("농지취득자격증명", "대법원 2006.1.27. 선고 2005다59871 (농취증 반려): '농취증 발급 실패 시 위험.' 👉 [방어 특약]: 매수인의 농지취득자격증명 발급이 관할 관청으로부터 반려될 경우, 위약금 없이 매매계약을 즉시 해제한다."),
                ("기획부동산", "대법원 2014.12.11. 선고 2014다21714 (공유물 분할 불능): '지분 쪼개기 사기.' 👉 [방어 특약]: 해당 토지의 분할 등기가 개발행위허가 제한으로 불가능함이 확인될 경우 매매계약을 전면 취소한다.")
            ],
            "숙박업": [
                ...
                ("소방시설", "대법원 2011.8.25. 선고 2010다8992 (소방시설 미비 화재): '소방 필증 없는 여관 책임.' 👉 [방어 특약]: 본 매물의 다중이용업소 소방완비증명서(소방필증) 명의 변경 불가 시 양수인은 양도양수 계약을 즉시 해제할 수 있다.")
            ],
            "오피스텔": [
                ...
                ("공용관리비", "대법원 2012.5.10. 선고 2012다11604 (체납 관리비 승계): '전 소유자 공용 관리비 승계.' 👉 [방어 특약]: 매도인/임대인은 잔금일 기준 미납된 관리비 전액을 정산하며, 추후 징수되는 과거분 체납 관리비는 매도인이 책임진다.")
            ],
            "빌딩": [
                ("지역주택조합", "대법원 2019.11.14. 선고 2019다280375 (지주택 토지확보율 기망): '토지 확보 거짓말 사기.' 👉 [방어 특약]: 조합측이 제시한 토지사용승낙서 및 소유권 확보율이 단 1%라도 사실과 다를 경우, 가입 계약 원천 무효 및 납입금 전액 즉시 반환한다."),
                ("지식산업센터", "대법원 (불법 용도 변경): '지산 일반 식당 불법.' 👉 [방어 특약]: 해당 호실이 지원시설구역이 아니어 식당/도소매 영업 허가가 불가능할 시 분양/임대차 계약은 즉시 파기한다.")
            ],
            "경매": [
                ("매각허가결정", "대법원 2013.2.14. 선고 2012다49292 (매각물건명세서 누락): '숨은 가처분 시 매각 취소.' 👉 [액션 지시]: 법원 매각물건명세서를 다시 열람하여 '인수할 권리' 칸에 가처분, 법정지상권이 적혀 있는지 눈으로 2번 확인하십시오."),
                ("유치권", "대법원 2011.12.22. 선고 2011다84298 (가짜 공사대금 유치권): '허위 유치권 경매 방해죄.' 👉 [액션 지시]: 유치권 신고인의 사업자등록증과 세금계산서 발행 내역을 확인해 허위 공사대금인지 밝혀내십시오.")
            ]
        }
    ...
    def fetch_naver_rtms(self, region: str, interest_type: str = ""):
        ...
        proxy_url = f"http://api.scraperapi.com?api_key={self._next_scraperapi_key()}&url={urllib.parse.quote(url)}"
        ...
        try:
            ...
            proxy_type_url = f"http://api.scraperapi.com?api_key={self._next_scraperapi_key()}&url={urllib.parse.quote(type_url)}"
            ...
            def process_ptp(p):
                ...
                try:
                    ...
                    if res3.status_code != 200:
                        proxy_rtms = f"http://api.scraperapi.com?api_key={self._next_scraperapi_key()}&url={urllib.parse.quote(rtms_url)}"
